...
class MaterialSeederLatest extends Seeder
{
    public function run(): void
    {
        ...
        // wipe & re-seed (safe even with FKs)
        DB::statement('SET FOREIGN_KEY_CHECKS=0;');
        DB::table('materials')->truncate();
        DB::statement('SET FOREIGN_KEY_CHECKS=1;');

        // NOTE: costs are per square inch
        $materials = [
            // Stickers / Films
            ['name' => 'PVC Blockout Sticker',          'cost' => 0.006900, 'type' => 'Stickers / Films'],
        ...
        // normalize to keyed rows
        $rows = [];
        foreach ($materials as $m) {
            if (is_array($m) && array_is_list($m) && count($m) === 3) {
                [$name, $cost, $type] = $m;
            } else {
                $name = $m['name'];
                $cost = $m['cost'];
