...
class AdminOrderController extends Controller
{
    public function orders(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return view('admin.partials.orders-table', compact('orders', 'isHead'))->render();
        }

        return view('admin.orders', compact('orders', 'metrics', 'isHead', 'statusRaw'));
    }
    ...
    public function showAssign(Order $order)
    {
        ...
        return view('admin.orders.assign', compact('order', 'artists'));
    }
    ...
    public function show(Order $order)
    {
        ...
        return view('admin.orders.shows', compact(
            'order',
            'attachments',
            'headerAttachments'
        ));
    ...
    {
        $user = Auth::user();
        if (!$user || !($user->hasRole('artist') || $user->hasRole('head-artist') || $user->hasRole('admin'))) {
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $assignee = null;
            if (auth()->user()->hasRole('admin') && $request->filled('assignee_artist_id')) {
            ...
            if ($actor->hasRole('admin') && isset($assignee) && $assignee && $assignee->hasRole('artist')) {
                $messageForAssignee = "You have been **assigned** a new order {$order->order_number} by {$actorName} "
                                    . "(admin). {$productCount} Product(s) added. Deadline: {$deadlineTxt}.";
            }
            ...
            if (strtolower($actor->role) === 'admin' && isset($assignee) && $assignee && (strtolower($assignee->role) === 'artist' || strtolower($assignee->role) === 'head-artist')) {
    ...
    public function getOrders(Request $request)
    {
        ...
        return DataTables::of($orders)
            ...
            ->addColumn('actions', function ($order) {
                $editRoute = route('admin.orders.edit', $order->id);
    ...
    public function redoStore(Request $request, Order $order)
    {
        ...
        return redirect()->route('admin.orders')->with('success', 'Redo updated.');
    }
