...
class FurnishingHistoryController extends Controller
{
    public function index(Request $request)
    {
        ...
        // mm/dd/yyyy or any parseable value -> yyyy-mm-dd.
        ...
        // Editable redo children used to determine whether the trailing R is shown.
        $redoChildren = DB::raw('(
        ...
        // Build the base query for furnishing completion history.
        $base = DB::table('products as p')
            ...
            ->when($pid !== '', function ($qb) use ($pid, $formattedCodeWithoutRedoExpr) {
                ...
                $qb->where(function ($w) use ($pid, $like, $formattedCodeWithoutRedoExpr) {
                    // Exact numeric matches. Keep IDs as strings so PHP does not
                    // introduce an unnecessary integer-size limitation.
                    if (ctype_digit($pid)) {
                        $w->orWhere('o.id', $pid)
                            ->orWhere('o.redo', $pid)
                    ...
                    // Partial and formatted-code matches.
        ...
        // Unified keyword search: order title, company, product, printer,
        // remarks, order number, ProductID, and formatted product code.
        if ($q !== '') {
            $like = "%{$q}%";
