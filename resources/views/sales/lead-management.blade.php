...
<script>
  ...
  $(document).ready(function() {
    ...
    $('#leadTable').on('change', '.status-dropdown', function(e){
      e.stopPropagation();
      $.ajax({
        url: '{{ route('leads.update.status', ['id' => ':id']) }}'.replace(':id', $(this).data('id')),
        type: 'POST',
    ...
    $('#leadTable').on('change', '.opportunity-dropdown', function(e){
      e.stopPropagation();
      $.ajax({
        url: '{{ route('leads.update.opportunity', ['id' => ':id']) }}'.replace(':id', $(this).data('id')),
        type: 'POST',
    ...
    $(document).on('click', '#confirmReminderDoneBtn', function(){
      ...
      $.ajax({
        url: '{{ route('leads.confirm.reminder.status', ['id' => ':leadId', 'reminderId' => ':reminderId']) }}'
    ...
    $('#leadTable').on('click', '.view-attachments', function(e){
      e.stopPropagation();
      $.ajax({
        url: '{{ route('leads.attachments', ['id' => ':id']) }}'.replace(':id', $(this).data('id')),
        type: 'GET',
