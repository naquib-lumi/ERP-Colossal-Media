                                        ...
                                        <ul class="list-group list-group-flush">
                                            @forelse ($lead->reminders as $reminder)
                                                <li
                                                    ...
                                                    <span
                                                        class="badge bg-{{ $reminder->status == 'overdue' ? 'danger' : ($reminder->status == 'upcoming' ? 'warning' : 'success') }} rounded-pill">
        ...
        @forelse ($lead->meetings->sortBy('start_time') as $meeting)
            <li class="list-group-item d-flex justify-content-between align-items-start">
                <div class="flex-grow-1">
                    ...
                    @if($meeting->note)
                        <br><small class="text-muted"><strong>Desc:</strong> {{ $meeting->note }}</small>
                    @endif
                </div>
 <div class="ms-2">
<span class="badge bg-{{ $meeting->status == 'scheduled' ? 'warning' : ($meeting->status == 'completed' ? 'success' : 'danger') }} rounded-pill mb-1 d-block">{{ ucfirst($meeting->status) }}</span>
    <button class="btn btn-sm btn-primary edit-meeting me-1" data-id="{{ $meeting->id }}" data-bs-toggle="modal" data-bs-target="#meetingModal">Edit</button>
    <select class="form-select form-select-sm status-update" data-id="{{ $meeting->id }}" style="width: auto;">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="meetingForm">
                    ...
                    <input type="hidden" id="meetingLeadId" name="lead_id" value="{{ $lead->id }}">
                    <div class="mb-3">
...
$(document).ready(function() {
    ...
    // Edit meeting - populate form
    $('.edit-meeting').on('click', function() {
        let id = $(this).data('id');
