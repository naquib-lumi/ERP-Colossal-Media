...
<div class="container-fluid py-4 px-4" style="max-width:1200px;margin:0 auto">
  ...
  {{-- ===== Filter Card (furnishing) ===== --}}
  <div class="card shadow-soft mb-3 filter-card">
    <div class="card-body">
      <form method="GET" action="{{ route('furnishing.progress') }}">

        {{-- 顶部：标题 + Filter Me + Reset + Apply（同一排） --}}
        <div class="filter-head">
          <div class="title">
            <h6 class="mb-0 fw-semibold">Furnishing</h6>
          ...
          <div class="actions">
            ...
            <a href="{{ route('furnishing.progress') }}" class="btn btn-outline-secondary">
          ...
          <tr>
            ...
            @php
              ...
              $urlWith = function(array $overrides) use ($q) {
                return route('furnishing.progress', array_filter(array_merge($q, $overrides), fn($v)=>$v!==null && $v!==''));
              };
              // for actions sort
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('.clickable-row').forEach(row => {
    row.addEventListener('dblclick', () => {
      ...
      if (id) window.location.href = `/furnishing/jobs/${id}`;
    });
  });
});
