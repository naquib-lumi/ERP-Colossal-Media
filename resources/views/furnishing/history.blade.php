      ...
      <form method="GET" action="{{ route('furnishing.history') }}">
        <div class="card-body toolbar">
          <input
            ...
            placeholder="Search by Product ID, Product Name, or Remark">

          <div class="dates">
            {{-- Use native HTML date inputs; no calendar button needed --}}
            <input type="date" name="start"
              value="{{ old('start', (preg_match('/^\d{4}-\d{2}-\d{2}$/', $start ?? '') ? $start : '')) }}"
              class="form-control date-input">
            <span class="text-muted">to</span>
            <input type="date" name="end"
              value="{{ old('end', (preg_match('/^\d{4}-\d{2}-\d{2}$/', $end ?? '') ? $end : '')) }}"
              class="form-control date-input">
          </div>
