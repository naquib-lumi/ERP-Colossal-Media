...
<style>
  ...
  .col-qty{width:100px}.col-size{width:30rem}.col-bleed{width:20rem}.col-material{width:140px}.col-centre{width:80px}
  .col-lam{width:160px}.col-printer{width:170px}.col-cutter{width:180px}.col-assemble{width:80px}
  ...
  /* ===== Resizable table ===== */
  .resize-table{table-layout:fixed;width:100%;border-collapse:separate!important;border-spacing:0}
  .resize-table thead th{position:relative;overflow:visible;background-color:#e9ecef;border-right:1px solid #d3d3d3;border-bottom:1px solid #ccc}
  .resize-handle{position:absolute;top:0;right:-4px;width:8px;height:100%;cursor:col-resize;z-index:2}
  .resize-handle::after{content:"";position:absolute;top:0;bottom:0;left:3px;width:2px;background:transparent;transition:background .15s}
  .resize-handle:hover::after{background:#d0d5dd}
  .is-resizing *{cursor:col-resize!important}
  .resize-guide{position:fixed;top:0;bottom:0;width:1px;background:#94a3b8;pointer-events:none;z-index:9999;display:none}
  ...
  <div class="page-wrap" id="pageRoot">
    ...
    <div class="card soft mb-4">
      <div class="card-body">
        ...
        <div class="row g-4">
          ...
          <div class="col-12 col-lg-6">
            <dl class="dl">
              <dt>Assigned By</dt><dd>{{ $header->artist_name ?? '—' }}</dd>
              <dt>Received Date</dt><dd>{{ $dates['order_date'] ?? '—' }}</dd>
              <dt>Deadline</dt><dd><span class="muted">{{ $dates['deadline'] ?? '—' }}</span></dd>
        ...
        {{-- ===== Product Remarks（按图样式） ===== --}}
        <div class="mt-3">
          <div class="section-hd mb-2">
            <i class="bi bi-chat-square-text"></i> Product Remarks
          </div>

          @if(!empty($remarks) && count($remarks))
            @foreach($remarks as $r)
              @php
                $op   = strtolower((string)$r->operation ?: 'printing');
                $pill = $opLabel[$op] ?? ucfirst($op);
                $cls  = 'op-'. $op;
                $who  = $r->author_name ?: 'Unknown';
                $when = \Carbon\Carbon::parse($r->created_at)->format('Y-m-d');
              @endphp

              <div class="remark-card">
                <div class="head">
                  <span class="op-badge {{ $cls }}">{{ $pill }}</span>
                  <div class="meta">by <strong>{{ $who }}</strong> · {{ $when }}</div>
                </div>
                <div class="body">{{ $r->remark }}</div>
              </div>
            @endforeach
          @else
      ...
      <div class="card-body">
        <div class="subcard mb-3">
          <div class="subcard-head">
            ...
            <div class="right">
              <button class="btn-toggle-icon" data-toggle="subcard" data-target="p{{ $block['id'] }}-body">
          ...
          <div class="subcard-body" id="p{{ $block['id'] }}-body" data-block-product="{{ $block['id'] }}">
            <div class="table-responsive">
              @php $widthKey = 'items-cols-' . ($header->ProductID ?? $product_header['code'] ?? 'prod'); @endphp
        ...
        {{-- 隐藏提交表单 --}}
        <form id="saveForm" method="POST" action="{{ route('furnishing.jobs.save', $header->ProductID) }}" style="display:none">@csrf</form>
        ...
        <div class="card dlv-card mb-4">
          <div class="card-body">
            ...
            <div class="d-flex justify-content-between align-items-start mb-2">
              ...
              <div class="dlv-sub">Total: {{ number_format($tot['total']) }} · Delivery Plan: {{ number_format($tot['delivered']) }} · Remaining: {{ number_format($tot['remaining']) }}</div>
            </div>

            <div class="dlv-product">
              <div class="dlv-product-title">Product — {{ $block['product_header']['name'] ?? 'Product' }}</div>
              <div class="dlv-list">
                @foreach ($block['deliveries'] as $d)
                  ...
                  <div class="dlv-item">
                    ...
                    <div class="dlv-main">
                      <div class="dlv-head">
                        <span class="badge-method {{ $badgeClass }}"><i class="bi {{ $badgeIcon }}"></i> {{ $d['method_label'] }}</span>
                      </div>
                      <div class="dlv-fields">
                        <div class="field"><div class="label">Quantity</div><div class="value value-strong">{{ number_format($d['quantity']) }}</div></div>
                        <div class="field"><div class="label">Location</div><div class="value">{{ ($d['location'] ?? '') !== '' ? $d['location'] : '—' }}</div></div>
                        <div class="field"><div class="label">Delivery Date &amp; Time</div><div class="value">{{ $d['datetime'] ?: '—' }}</div></div>
                        @if(!empty($d['install'])) <div class="field"><div class="label">Installation Type</div><div class="value">{{ $d['install'] }}</div></div> @endif
                        @if(array_key_exists('cost',$d) && $d['cost'] !== null) <div class="field"><div class="label">Cost</div><div class="value">{{ number_format($d['cost'], 2) }}</div></div> @endif
                      </div>
                    </div>
                  </div>
                @endforeach
    ...
    <div class="card soft mb-4 edit-only">
      <div class="card-body">
        ...
        <div class="mt-2"><button id="btn-add-remark" type="button" class="btn btn-dark btn-sm"><i class="bi bi-plus-lg me-1"></i>Add Remark</button></div>
      </div>
    </div>
    @endif
    ...
    {{-- Accept / Reject forms --}}
    <form id="acceptForm" method="POST" action="{{ route('furnishing.orders.accept', $header->ProductID) }}" style="display:none">@csrf</form>
    <form id="rejectForm" method="POST" action="{{ route('furnishing.orders.reject', $header->ProductID) }}" style="display:none">@csrf<input type="hidden" name="reason" id="rejectReasonInput"></form>
  ...
  <div class="cx-wrap">
    <div class="cx-modal" role="dialog" aria-modal="true" aria-labelledby="acceptTitle">
      <div class="cx-header">
        ...
        <div id="acceptTitle" class="cx-title">Accept Furnishing Task</div>
...
<div id="modalReject" class="cx-mask" aria-hidden="true">
  <div class="cx-wrap">
    <div class="cx-modal" role="dialog" aria-modal="true" aria-labelledby="rejectTitle">
      <div class="cx-header">
        <div id="rejectTitle" class="cx-title">Reject Furnishing Task</div>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  document.getElementById('confirmAccept')?.addEventListener('click', () => { document.getElementById('acceptForm')?.submit(); });
  ...
  function syncCutterSelects(scopeEl){
    (scopeEl || document).querySelectorAll('.td-cutter').forEach(td => {
  ...
  (function initDynamicRemarks(){
    ...
    const ops = {
      'printing':'To Printing','furnishing':'To Furnishing','installation':'To Delivery & Installation',
      'courier':'To Courier','self_pickup':'To Self Pickup','artist':'To Artist'
    };
    ...
    function makeRow(){
      ...
      let options = '';
      for (const [val,label] of Object.entries(ops)) { options += `<option value="${val}">${label}</option>`; }
      d.innerHTML = `
        <select class="form-select form-select-sm remark-cat" style="max-width:180px">${options}</select>
        <input class="form-control form-control-sm remark-text" placeholder="Add your remark..." />
        <button type="button" class="btn btn-link text-muted p-0 remove-remark" title="Remove"><i class="bi bi-trash"></i></button>`;
      return d;
    }
    ...
    list.addEventListener('click', e => {
      const rm = e.target.closest('.remove-remark'); if (!rm) return;
  ...
  document.getElementById('btnSave')?.addEventListener('click', () => {
    const pid  = {{ (int)$header->ProductID }};
    const form = document.getElementById('saveForm');

    Array.from(form.querySelectorAll('input[type="hidden"]')).forEach(inp => { if (inp.name !== '_token') inp.remove(); });

    const block = document.querySelector(`[data-block-product='${pid}']`);
    ...
    rows.forEach(r => {
      ...
      if (!tx) return;
      const h1 = document.createElement('input'); h1.type='hidden'; h1.name=`remarks[${i}][operation]`; h1.value=op; form.appendChild(h1);
      const h2 = document.createElement('input'); h2.type='hidden'; h2.name=`remarks[${i}][remark]`;   h2.value=tx; form.appendChild(h2);
      i++;
    });
...
(function() {
  ...
  function initResizableTable(tbl){
    ...
    if (saved){
      try{ JSON.parse(saved).forEach((w,i)=>{ if(cols[i]) cols[i].style.width = w; }); }catch(e){}
    } else {
    ...
    function save(){ const widths = cols.map(c => c.style.width || (c.getBoundingClientRect().width + 'px')); localStorage.setItem(key, JSON.stringify(widths)); }
    function onMove(e){ if(!moving) return; const dx = e.clientX - startX; const newW = Math.max(60, startW + dx); cols[index].style.width = newW + 'px'; guide.style.left = (startX + dx) + 'px'; }
    function onUp(){ if(!moving) return; moving=false; document.body.classList.remove('is-resizing'); guide.style.display='none'; save(); window.removeEventListener('mousemove', onMove, true); window.removeEventListener('mouseup', onUp, true); }

    ths.forEach((th,i)=>{
      const handle = th.querySelector('.resize-handle');
      if (!handle || i === ths.length - 1) return;
      handle.addEventListener('mousedown', (e)=>{
        e.preventDefault(); startX = e.clientX; startW = parseFloat((cols[i].style.width || th.getBoundingClientRect().width)); index = i; moving = true;
        document.body.classList.add('is-resizing'); guide.style.display='block'; guide.style.left = startX + 'px';
        window.addEventListener('mousemove', onMove, true); window.addEventListener('mouseup', onUp, true);
      });
    });
  }

  document.addEventListener('DOMContentLoaded', ()=>{ document.querySelectorAll('.js-resize-table').forEach(initResizableTable); });
})();
</script>
