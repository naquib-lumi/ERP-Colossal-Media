...
<div class="container-fluid py-4 px-4" style="max-width:1200px;margin:0 auto">
  ...
  <div class="card shadow-soft mb-3 filter-card">
    <div class="card-body">
      <form method="GET" action="{{ route('artist.progress') }}">
        ...
        <div class="filter-head">
          ...
          <div class="actions">
            <a href="{{ route('artist.progress') }}" class="btn btn-outline-secondary">
          ...
          <tr>
            ...
            @php
              ...
              $urlWith = function(array $overrides) use ($q) {
                return route('artist.progress', array_filter(array_merge($q, $overrides), fn($v)=>$v!==null && $v!==''));
              };
              // for actions sort
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('.clickable-row').forEach(row => {
    row.addEventListener('dblclick', () => {
      ...
      if (id) window.location.href = `/artist/orders/${id}`;
    });
  });
});
