...
<div class="container-fluid py-4 px-4" style="max-width:1200px;margin:0 auto">
  ...
  <div class="card-body">
    ...
    <form class="row g-3 align-items-end" method="GET" action="{{ route('dispatchcontrol.dashboard') }}">
          ...
          <tbody>
            ...
            @forelse ($rows as $r)
              @php
                ...
                $end = $lastCompleted
                ? ($lastCompleted === 'delivery' ? 100 : $POS[$lastCompleted])
                : $start;
                }
...
document.addEventListener('DOMContentLoaded', () => {
  document.querySelectorAll('.clickable-row').forEach(row => {
    row.addEventListener('dblclick', () => {
      ...
      if (id) {
        // adjust route if your route name differs
        window.location.href = `/dispatchcontrol/job/${id}`;
      }
    });
  });
});
