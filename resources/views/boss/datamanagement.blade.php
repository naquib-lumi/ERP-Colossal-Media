...
.control{
  ...
  background:#fff;outline:none;color:var(--text);font-size:14px;
}
...
/* Chips */
.badge-chip{display:inline-flex;align-items:center;gap:6px;padding:5px 10px;border-radius:999px;background:var(--chip);color:#344054;font-weight:600;font-size:12px}
.usage-scroll{display:flex;gap:8px;flex-wrap:wrap}
...
.kebab:focus{outline:2px solid #E5E7EB;outline-offset:2px}

/* Modals（保持不变） */
.modal-backdrop-custom,.modal-backdrop-edit,.modal-backdrop-type{position:fixed; inset:0; background:rgba(17,24,39,.55); display:none; align-items:center; justify-content:center; z-index:1060;}
.modal-backdrop-custom.open,.modal-backdrop-edit.open,.modal-backdrop-type.open{display:flex}
        ...
        <div class="table-wrap">
          <div class="table-responsive">
            <table class="table align-middle" id="costingTable">
              <thead>
                <tr>
                  <th class="col-name text-uppercase small">Material Name</th>
                  <th class="col-type text-uppercase small">Machine Type</th>
                  <th class="col-unit text-uppercase small">Unit Cost (per sq inch)</th>
                  <th class="col-qty text-uppercase small">Used Quantity</th>
                  <th class="col-total text-uppercase small">Total Cost</th>
                  <th class="col-usage text-uppercase small">Past Usage</th>
...
function rowTpl(d, idx){
  ...
  return `
    <tr>
      <td class="col-name fw-500 text-dark">${d.name}</td>
      <td class="col-type text-muted">${d.type}</td>
      <td class="col-unit">${formatUnit(d.unit)}</td>
      <td class="col-qty">${fmtInt(d.qty)}</td>
      <td class="col-total">${formatMoney(d.total)}</td>
      <td class="col-usage"><div class="usage-scroll">${chips}${extra}</div></td>
...
function openAddMaterial(){
  ...
  $matName.value=''; $matCategory.selectedIndex=0; $matUnit.value=''; $matErr.style.display='none';
  setTimeout(()=> $matName.focus(), 50);
}
...
document.getElementById('btnMatSave').addEventListener('click', ()=>{
  ...
  const type=$matCategory.value || 'Paper Materials';
  const unit=Number($matUnit.value);
  if(!name){ return showErr('Please enter material name.'); }
  if(!(unit>=0)){ return showErr('Please enter a valid unit cost.'); }
  DATA.unshift({ name, type, unit, qty:0, total:0, usage:[], days:0 });
  state.page=1; render(); closeAddMaterial();
});
...
elBody.addEventListener('click', e=>{
  ...
  editNewName.value=item.name;
  editCurrent.value=`${formatUnit(item.unit)} / sqft`.replace('RM ','RM ');
  editNewUnit.value=item.unit.toFixed(4);
...
document.getElementById('editSave').addEventListener('click', ()=>{
  ...
  const val=Number(editNewUnit.value);
  if(!(val>=0)){ editErr.textContent='Please enter a valid unit cost.'; editErr.style.display='block'; return; }
  const newName=editNewName.value.trim(); if(newName) DATA[editingIndex].name=newName;
  DATA[editingIndex].unit=val; render(); closeEdit();
});
...
document.getElementById('typeSave')?.addEventListener('click', ()=>{
  const name=(typeName.value||'').trim();
  if(!name){ typeErr.textContent='Please enter a material type name.'; typeErr.style.display='block'; return; }
  const exists=TYPES.some(t=>t.toLowerCase()===name.toLowerCase());
  if(exists){ typeErr.textContent='This type already exists.'; typeErr.style.display='block'; return; }
  TYPES.push(name); refreshTypeOptions(name); state.type='all'; render(); closeTypeModal();
});
