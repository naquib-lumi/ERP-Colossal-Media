...
<form id="order-form" action="{{ route('boss.orders.store') }}" method="POST" enctype="multipart/form-data" novalidate>
    @csrf
    ...
    <div class="row g-4">
                    ...
                    <div class="card mt-4">
                        ...
                        <div class="card-body">
                            ...
                            <div class="d-flex align-items-center justify-content-between mb-2">
                                ...
                                <a id="csvTemplateBtn" href="{{ route('boss.orders.csv_template') }}" class="btn btn-link p-0 text-decoration-none">
        ...
        <div class="col-12">
            <div class="bg-body position-sticky bottom-0 border-top py-3 d-flex gap-2 justify-content-end" style="z-index: 10">
                <a type="button" class="btn btn-outline-secondary" href="{{ route('boss.orders') }}">Cancel</a>
...
<script>
window.currentUserRole = '{{ auth()->user()->hasRole("boss") ? "boss" : "artist" }}';
$(function () {
  /***********************
   * LEAD SELECT (Select2)
   ***********************/
  ...
  $leadSel.select2({
    ...
    ajax: {
      url: @json(route('boss.orders.leads.search')),
      dataType: 'json',
      delay: 250,
  ...
  $leadSel.on('select2:select', function (e) {
    ...
    if (m.company_name || m.phone || m.email) {
      ...
      const url = @json(route('boss.orders.leads.get', ['id' => 'ID']));
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  form.addEventListener('submit', function (e) {
    ...
    if (role !== 'boss') return; // only validate for head-artist
