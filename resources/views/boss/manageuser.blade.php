...
<div class="container-fluid py-4 px-4">
  <div class="page-wrap">
    ...
    <div class="table-section">
      {{-- Toolbar：GET 方式，保留筛选 --}}
      <form id="filterForm" class="section-toolbar" method="GET" action="{{ route('boss.manageuser') }}">
                ...
                @forelse($users as $user)
                  <tr>
                    ...
                    <td class="col-actions">
                      ...
                      <form action="{{ route('boss.user.disable',$user->id) }}" method="POST" style="display:inline"
...
function prepareCreate(){
  form.action = @json(route('boss.user.store'));
...
document.querySelector('tbody').addEventListener('click', function(e){
  ...
  const u = {
    ...
    update: @json(route('boss.user.update', ['user' => '___ID___'])).replace('___ID___', btn.dataset.id)
  };
