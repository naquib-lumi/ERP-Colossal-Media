          ...
          @forelse($users as $user)
            ...
            <tr data-role="{{ $user->role }}" data-status="{{ strtolower((string)($user->status ?? '')) }}">
              ...
              <td class="col-actions">
                ...
                <form action="{{ route('boss.user.disable',$user->id) }}" method="POST" style="display:inline" class="disable-form">
                  @csrf @method('PATCH')
                  <button type="submit" class="icon-btn disable-btn" title="{{ $active? 'Disable':'Enable' }}">
                    <i class="bi bi-{{ $active? 'person-dash':'person-check' }}"></i>
                  </button>
                </form>
                <form action="{{ route('boss.user.resetpassword',$user->id) }}" method="POST" style="display:inline" class="reset-form">
                  @csrf @method('PATCH')
...
<div class="modal-backdrop-user" id="userModal" aria-hidden="true" role="dialog" aria-modal="true">
  <div class="modal-user" role="document">
    ...
    <form id="userForm" method="POST" action="{{ route('boss.user.store') }}">
      @csrf
...
(() => {
  ...
  function prepareCreate() {
    $('#userForm').action = '{{ route('boss.user.store') }}';
  ...
  $$('button[data-mode="edit"]').forEach(btn => {
    btn.addEventListener('click', () => {
      const u = {
        ...
        update: '{{ route('boss.user.update', ['user' => '___ID___']) }}'.replace('___ID___', btn.dataset.id)
      };
      prepareEdit(u);
      openModal();
    });
