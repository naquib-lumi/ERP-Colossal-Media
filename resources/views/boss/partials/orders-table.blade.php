...
@php
  ...
  $deadlineSortUrl = route('boss.orders', array_merge(request()->query(), ['deadline_sort' => $nextSort]));
@endphp
<style>
  .bg-amber {
...
@forelse ($orders as $order)
  ...
  <tr data-href="{{ route('boss.orders.show', $order->id) }}" style="cursor:pointer;">
    <td>
    ...
    <td>
      @if($isHead)
        ...
        @if($needsAssign && !$isArchived)
          <a href="{{ route('boss.orders.assign.show', $order->id) }}"
    ...
    <td class="text-end">
      <div class="d-inline-flex align-items-center gap-3">
        {{-- View (always enabled) --}}
        <a href="{{ route('boss.orders.show', $order->id) }}" class="text-secondary fw-bold" title="View">
          <i class="bx bx-show fs-5"></i>
        </a>

        {{-- Edit (disabled for completed/rejected/archived) --}}
        @if($canEdit)
          <a href="{{ route('boss.orders.edit', $order->id) }}" class="text-secondary fw-bold" title="Edit">
        ...
        @if(!$isArchived && !$reportBlocked)
          <a href="{{ route('boss.orders.redo.create', $order->id) }}" class="text-secondary fw-bold" title="Report">
