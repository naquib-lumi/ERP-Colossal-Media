        ...
        <div class="card-body">
          ...
          <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3">
            ...
            <div id="ordersToolbar" class="d-flex align-items-center gap-2 flex-wrap">
              ...
              <a href="{{ route('boss.orders') }}" class="btn btn-outline-secondary">Reset</a>
              <button id="exportExcel" type="button" class="btn btn-dark">
                <i class="bx bx-export me-1"></i> Export
              </button>
              <a href="{{ route('boss.orders.create') }}" class="btn btn-light text-primary">Add Order</a>
            </div>
          </div>

          <div class="w-100 border rounded-3 px-3 py-3">
            <!-- 第一排：Order ID + Salesperson + Search + Status -->
            <form id="ordersFilterForm" method="GET" action="{{ route('boss.orders') }}">
              <div class="row g-2 align-items-center">
                ...
                <div class="col-12 col-lg-2">
                  <select name="status" class="form-select">
                    ...
                    <option value="redo"    {{ $statusVal==='redo' ? 'selected' : '' }}>Redo</option>
                    @if($isHead ?? false)
          ...
          <div class="table-responsive" id="orders-table-wrapper">
            @include('boss.partials.orders-table', ['orders' => $orders])
          </div>
        </div>
      </div>
...
<script>
  ...
  (function() {
    ...
    const form = document.getElementById('ordersFilterForm') || document.querySelector('form[action*="boss/orders"]');
    if (!form) return;
