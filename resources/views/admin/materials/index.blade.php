...
<style>
  ...
  .inactive { color: var(--muted); opacity: 0.7; }
</style>
<div class="page-wrap">
  <div class="card">
    <div class="card-hd">
...
<div id="mdlTypes" class="x-mask" aria-hidden="true">
  <div class="x-dialog" style="width:800px; max-width:90vw">
    ...
    <div class="x-bd" style="padding:0">
     <div class="toolbar" style="padding:14px 20px;border-bottom:1px solid var(--border); display:flex; justify-content:flex-end;">
...
(() => {
  ...
  function applyFilter(tableId, qId, filterId){
    const q = ($(qId)?.value || '').trim().toLowerCase();
    const type = $(filterId)?.value || 'all';
    let shown = 0;
    $$(tableId + ' tbody tr').forEach(tr=>{
      const name = tr.children[0].innerText.toLowerCase();
      const tVal = tr.getAttribute('data-type') || '';
      const ok = (!q || name.includes(q)) && (type==='all' || tVal===type);
  ...
  function applyFilters(){
    const shownMat = applyFilter('#tbl', '#q', '#typeFilter');
    const cs = $('#countShown');
    if (cs) cs.textContent = shownMat;
    applyFilter('#tblTypes', '#qType', null);
  }
  document.addEventListener('click', (e)=>{
    ...
    const delTypeBtn = t.closest('.btnDeleteType');
    if (delTypeBtn){
      Swal.fire({
        title: 'Are you sure?',
        ...
        if (result.isConfirmed) {
          const id = delTypeBtn.dataset.id;
          fetch(`/admin/material-types/${id}`, {
            method: 'DELETE',
            headers: {'X-CSRF-TOKEN': csrf}
          }).then(res => res.json()).then(data => {
            if(data.success){
              delTypeBtn.closest('tr').remove();
              Swal.fire('Deleted!', 'The material type has been deleted.', 'success');
              location.reload(); // to update type selects
            } else {
              Swal.fire({ icon: 'error', title: 'Error deleting' });
            }
          }).catch(() => Swal.fire({ icon: 'error', title: 'Error' }));
        }
      });
              ...
              tr.innerHTML = `
                ...
                <td style="text-align:right; position:relative">
                  ...
                  <div class="dropdown-menu">
                    ...
                    <button class="dropdown-item text-danger btnDeleteType" data-id="${data.type.id}"><i class="bi bi-trash me-2"></i> Delete</button>
                  </div>
                </td>
              `;
    ...
    if (t.closest('#btnSaveMaterial')) {
      ...
      fetch('{{ route('admin.materials.store') }}', {
        ...
        if (data.success) {
          ...
          tr.classList.add(m.active ? '' : 'inactive');
          tr.innerHTML = `
            <td>${m.materialName}</td>
            <td>${m.materialType.name}</td>
            <td>RM ${Number(m.unitCost).toFixed(4)} SQ INCH</td>
            ...
            <td style="text-align:right; position:relative">
              ...
              <div class="dropdown-menu">
                <button class="dropdown-item btnEdit" data-id="${m.MaterialID}" data-name="${m.materialName}" data-type="${m.material_type_id}" data-cost="${m.unitCost}">
                  <i class="bi bi-pencil me-2"></i> Edit
                </button>
                <button class="dropdown-item btnToggle" data-id="${m.MaterialID}" data-active="${m.active}">
                  <i class="bi bi-toggle${m.active ? 'on' : 'off'} me-2"></i> ${m.active ? 'Deactivate' : 'Activate'}
                </button>
      ...
      fetch(`/admin/materials/${id}`, {
        ...
        if(data.success){
          ...
          if(tr){
            ...
            tr.children[1].textContent = data.material.materialType.name;
            tr.children[2].textContent = `RM ${Number(postData.qeNew).toFixed(4)} SQ INCH`;
            tr.dataset.type = postData.qeType;
            const btn = tr.querySelector('.btnEdit');
            btn.dataset.name = tr.children[0].textContent;
            btn.dataset.type = postData.qeType;
  ...
  ['input','change'].forEach(ev=>{
    document.addEventListener(ev, (e)=>{
      if (['q','typeFilter','qType'].includes(e.target.id)) applyFilters();
    });
  });
